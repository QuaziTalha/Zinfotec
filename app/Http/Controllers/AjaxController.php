...
class AjaxController extends Controller
{
    ...
    public function ClientInsert(Request $req)
    {
        $validator = Validator::make($req->all(), [
            'logo' => ($req->input('client_id') != "") ? 'image' : 'required|image'
        ]);
        ...
        try {
            if ($req->hasFile('logo')) {
                $image = $req->file('logo');
                $imagename = Str::random(20) . '.' . $image->getClientOriginalExtension();
                $path = base_path('public/client/' . $imagename);
                file_put_contents($path, file_get_contents($image));
                $data['logo'] = $imagename;
            }
            if ($req->input('client_id') != "") {
                $old = DB::table('clients')->where('client_id', $req->input('client_id'))->first();
                if (file_exists(base_path('public/client/' . $old->logo))) {
                    unlink(base_path('public/client/' . $old->logo));
                }
                $insert = DB::table('clients')->where('client_id', $req->input('client_id'))->update($data);
            } else {
    ...
    public function ClientFetch(Request $req)
    {
        if ($req->input('id')) {
            $data = DB::table('clients')->where("client_id", $req->input('id'))->get();
        } else {
            $data = DB::table('clients')->get();
        }
        return $data;
    ...
    public function ClientDelete(Request $req)
    {
        $data = DB::table('clients')->where('client_id', $req->input('id'))->first();
        if (file_exists(base_path('public/client/' . $data->gallery_image))) {
            unlink(base_path('public/client/' . $data->gallery_image));
        }
        $delete = DB::table('clients')->where('client_id', $req->input('id'))->delete();
        return response()->json(["success" => true, "message" => "Gallery deleted Successfully"]);
    }
