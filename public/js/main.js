(function($) {
    ...
    $('.vertical-counter, .counter, .progress-bar, .pie-chart-style-01, .attractive-hover, .splitting-animation, .section-dark, footer, [data-anime], [data-fancy-text]').each(function() { $(this).appear().trigger('resize'); });
    initScrollNavigate();
    slideboxstyle();
    animateCounters();
    setSpaceInTeamStyle();
    ...
    $('.navbar-collapse.collapse').on('show.bs.collapse', function(e) {
        ...
        var windowHeight = getWindowHeight();
        if ($('.navbar-modern-inner').length > 0 || $('.navbar-full-screen-menu-inner').length > 0) { $('.navbar-collapse-clone').css('max-height', windowHeight); } else { $('.navbar-collapse-clone').css('max-height', (windowHeight - headerHeight)); }
    ...
    $(document).on('click', '.header-push-button .push-button', function() { $('html, body').toggleClass('show-menu'); });
    $(document).on('click', '.close-menu', function() {
        $('html, body').removeClass('show-menu');
        $('.sub-menu.show').collapse('hide');
    });
    $(document).on('click', '#navbar-menu .navbar-toggler', function() { $('.sub-menu.show').collapse('hide'); });
    ...
    $('.dropdown').on('mouseenter touchstart', function(e) {
        ...
        if (getWindowWidth() >= menuBreakPoint) { if (_this.hasClass('open') && _this[0] == $(e.target).parent()[0]) { let href = $(_this).find('.nav-link').attr('href'); if (href != 'undefined' && href.indexOf('javascript') === -1 && typeof undefined !== typeof href && href != '#') { window.location.href = href; } } }
        _this.addClass('open');
        menuPosition(_this);
        if (getWindowWidth() > menuBreakPoint) {
            if ($(e.target).siblings('.dropdown-menu').length) { e.preventDefault(); }
    ...
    if ($.fn.stick_in_parent !== undefined && $.fn.stick_in_parent !== null) { if ($('.left-sidebar-wrapper').length) { $('.left-sidebar-wrapper .left-sidebar-nav').stick_in_parent({ recalc: 1 }); } }
    ...
    function getHeaderHeight() {
        var headerHeight = 0;
        if ($('.header-top-bar').length) { headerHeight = headerHeight + $('.header-top-bar').outerHeight(); }
    ...
    function menuPosition(element) {
        ...
        if (element.parent().hasClass('dropdown-menu') && element.parents('.simple-dropdown')) {
            ...
            element.children('.dropdown-menu').each(function() {
                ...
                } else if (multiDepth < 1) { dropdownWidth = dropdownWidth + element.outerWidth(); }
            });
    ...
    if (typeof imagesLoaded === 'function') {
        $('.portfolio-wrapper, .shop-wrapper').each(function() {
            ...
            _this.imagesLoaded(function() {
                ...
                if (typeof $.fn.isotope === 'function') { var _thisIsotop = _this.isotope({ layoutMode: 'masonry', itemSelector: '.grid-item', percentPosition: true, stagger: 0, masonry: { columnWidth: '.grid-sizer', } }); }
                isotopeObjs.push(_this);
                if (typeof $.fn.isotope === 'function') { var currentPortfolioActive = $('.portfolio-filter > li.active > a').attr('data-filter'); if (currentPortfolioActive != '' && currentPortfolioActive != undefined) { _this.isotope({ filter: currentPortfolioActive }); } }
            });
            _this.on('arrangeComplete', function() { reInitSkrollr(); });
            $('a[data-bs-toggle="tab"]').on('shown.bs.tab', function(e) { _this.isotope({ transitionDuration: 0 }); });
        });
    }
    $(document).on('click', '.portfolio-filter > li > a', function() {
        ...
        if (typeof portfolioFilter.isotope === 'function') { portfolioFilter.isotope({ filter: selector }); }
            ...
            if (typeof TweenLite !== "undefined") {
                document.addEventListener("mousemove", function(e) {
                    let posX = e.clientX + 20,
                        posY = e.clientY + 20;
                    TweenLite.to(_self.querySelector(".hover-reveal"), .6, { x: posX + imgWidth > windowWidth ? e.clientX - imgWidth : posX, y: posY + imgHeight > windowHeight ? e.clientY - imgHeight : posY, })
                })
            }
        });
    }
            ...
            initEvents() {
                ...
                this.DOM.menuLinks.forEach((item, i) => {
                    ...
                    item.addEventListener('mouseenter', (e) => {
                        ...
                        if (typeof TweenMax !== 'undefined' && typeof TweenMax !== null) { if (item !== active_item) { TweenMax.fromTo(displaceMentEl, 1.2, { attr: { scale: 50 } }, { attr: { scale: 0.3 } }); } }
    ...
    if (typeof imagesLoaded === 'function') {
        $('.blog-wrapper').each(function() {
            ...
            _this.imagesLoaded(function() {
                _this.removeClass('grid-loading');
                if (typeof $.fn.isotope === 'function') { _this.isotope({ layoutMode: 'masonry', itemSelector: '.grid-item', percentPosition: true, stagger: 0, masonry: { columnWidth: '.grid-sizer', } }); }
                isotopeObjs.push(_this);
                var currentBlogActive = $('.blog-filter > li.active > a').attr('data-filter');
                if (currentBlogActive != '' && currentBlogActive != undefined) { _this.isotope({ filter: currentBlogActive }); }
            });
        });
    }
    ...
    $('.box-hover').on('mouseenter', function(e) { $(this).find('.hover-text').slideDown(400); }).on('mouseleave', function(e) { $(this).find('.hover-text').slideUp(400); });
    if (typeof imagesLoaded === 'function') {
    ...
    if (typeof $.fn.magnificPopup === 'function') {
        ...
        $('.portfolio-box [data-group], .gallery-box [data-group]').each(function() {
            ...
            if (!lightboxgallerygroups[id]) { lightboxgallerygroups[id] = []; }
    ...
    $('.checkout-accordion label input').on('click', function(e) { var collapseId = $(this).parent().find('a').attr('href'); if ($(this).prop("checked")) { $(collapseId).collapse('show'); } else { $(collapseId).collapse('hide'); } });
    ...
    $('[data-tab="review-tab"]').on('shown.bs.tab', function(e) { $('.progress-bar').each(function() { $(this).appear().trigger('resize'); }); });
    ...
    $('.vertical-counter').each(function() {
        ...
        for (var i = 0; i < valueLength; i++) { _this.append('<span class="vertical-counter-number"><ul><li>0</li><li>1</li><li>2</li><li>3</li><li>4</li><li>5</li><li>6</li><li>7</li><li>8</li><li>9</li></ul></span>'); }
    ...
    if ($('.vertical-counter').length) {
            ...
            if (!$(this).hasClass('appear')) {
                ...
                if ($(window).scrollTop() + getWindowHeight() >= $('.vertical-counter').offset().top) {
                    $(this).find('.vertical-counter-number').each(function() {
                        ...
                        if (value <= 9) { anime({ targets: this.querySelector('ul'), translateY: [0, `${-value*10}%`], duration: 2000, easing: 'easeOutCubic' }); }
                    });
                }
            }
        });
        $('a[data-bs-toggle="tab"]').on('shown.bs.tab', function(e) {
            ...
            activeTabPane.find('.vertical-counter').each(function() {
                ...
                if (value <= 9) { _this.find('ul').css({ 'transform': 'translateY(-' + value * 10 + '%)' }); }
            });
        });
    }
    $('[data-tab="counter"]').on('shown.bs.tab', function(e) { $('.vertical-counter').each(function() { $(this).appear().trigger('resize'); }); });
    ...
    function animateCounters() {
        $('.counter').each(function(options) {
            ...
            if (typeof $.fn.countTo === 'function') { _this.countTo(options); }
        });
    }
    ...
    function setSpaceInTeamStyle() {
        $('.team-style-01').each(function() {
            ...
            setTimeout(function() { figure.css({ 'padding-bottom': figcaption.outerHeight() }); }, 200);
        });
    }
    ...
    function setSpaceInInteractiveBannerStyle() {
        $('.interactive-banner-style-02').each(function() {
            let _this = $(this),
                figure = _this.find('figure'),
                figcaption = _this.find('figcaption');
            setTimeout(function() { figure.css({ 'padding-bottom': figcaption.outerHeight() }); }, 500);
        });
    }
    ...
    function animeAnimation(target, options) {
        ...
        function applyTransitionStyles(elements) {
            for (let i = 0; i < elements.length; i++) {
                ...
                if (options.willchange) { element.style.willChange = 'transform'; }
            }
        }
            ...
            complete: function() {
                if (options.el) {
                    ...
                    for (let i = 0; i < target.children.length; i++) {
                        ...
                        if (target.classList.contains('grid')) { element.style.transition = 'none'; }
                    }
                    if (options.el === "lines") {
                        for (let i = 0; i < target.children.length; i++) {
                            const element = target.children[i];
    ...
    const slideTextAnimation = (target, options) => {
        ...
        tmp.style.WebkitTransform = tmp.style.transform = 'scaleX(0)';
        tmp.style.WebkitTransformOrigin = tmp.style.transformOrigin = '0% 50%';
        if (options.direction === 'left') { tmp.style.WebkitTransformOrigin = tmp.style.transformOrigin = '100% 50%'; }
    ...
    const smoothWaveTextAnimation = (target, options) => {
        ...
        smoothWaveText.add({ targets: target.querySelectorAll('.anime-text > .word > .char'), opacity: [0, 1], translateY: direction === 'down' ? [-50, 0] : [50, 0], duration: 500, easing: 'easeOutQuad', delay: anime.stagger(speed ? speed : 40, { direction: 'reverse' }), }).add({ targets: target.querySelectorAll('.anime-text .word .char'), opacity: content.length > 1 ? [1, 0] : [1, 1], translateY: content.length > 1 ? (direction === 'down' ? 50 : -50) : 0, duration: 500, easing: 'easeOutQuad', delay: anime.stagger(speed ? speed : 40, { start: duration - 1000, direction: 'reverse' }) });
    }
    const rotateTextAnimation = (target, options) => {
        ...
        rotateText.add({ targets: target.querySelectorAll('.anime-text > .word > .char'), opacity: [0, 1], rotateX: [-70, 0], duration: 150, delay: anime.stagger(speed ? speed : 50), easing: "linear" }).add({ targets: target.querySelectorAll('.anime-text > .word > .char'), opacity: content.length > 1 ? 0 : 1, rotateX: content.length > 1 ? [0, 70] : [0, 0], duration: 150, delay: anime.stagger(speed ? speed : 50, { start: duration - 1500 }), easing: "linear" });
    }
    ...
    const fadeTextAnimation = (target, options) => {
        let duration = options.duration ? options.duration : 3000,
            ...
            speed = options.speed,
            fade = anime.timeline();
        fade.add({ targets: target.querySelectorAll('.anime-text > .word > .char'), translateY: [100, 0], translateZ: 0, opacity: [0, 1], easing: "easeOutExpo", delay: anime.stagger(speed ? speed : 30) }).add({ targets: target.querySelectorAll('.anime-text > .word > .char'), translateY: content.length > 1 ? [0, -100] : [0, 0], opacity: content.length > 1 ? [1, 0] : [1, 1], easing: "easeInExpo", delay: anime.stagger(speed ? speed : 40, { start: duration - 3000 }) });
    }
    ...
    $('[data-fancy-text]').each(function() {
        ...
        if (ftOptions) {
            if (_this.hasClass('swiper-parallax-fancy-text')) { FancyTextDefault(this, ftOptions); } else {
    ...
    instagramWrapperItems.forEach(function(instagramWrapperItem) {
        var token = 'IGQWRPZA29XcnJtQ3lydUp4dHNvX0cwX1pSMzRlMm5sOGJ0bXpPS3h5dEVIN2ozQVkwb0VGMVNMNlh4eVBVUVN5WnBVR2tVeGdnbER5dWhJUUxCZAGo1TXBuRkxmcWU2dW9fYkJDaVRSM0RCNlFQUTZAmOFJKOUJsTnMZD',
            ...
            _html = _this.html(),
            outputHTML = '';
        if (typeof(slider) !== 'undefined' && slider !== null) { _this.html(''); }
        $.ajax({
            ...
            success: function(response) {
                ...
                if (typeof(slider) !== 'undefined' && slider !== null) { var sliderOptions = $.parseJSON(slider); var swiperObj = instagramWrapperItem.parentElement; if (typeof Swiper === 'function') { new Swiper(swiperObj, sliderOptions); } } else {
                    _this.imagesLoaded(function() {
                        _this.removeClass('grid-loading');
                        if (typeof $.fn.isotope === 'function') { _this.isotope({ layoutMode: 'masonry', itemSelector: '.grid-item', percentPosition: true, stagger: 0, masonry: { columnWidth: '.grid-sizer', } }); }
                        isotopeObjs.push(_this);
    ...
    if (typeof $.fn.magnificPopup === 'function') {
        if ($('#subscribe-popup').length > 0) {
            ...
            if (getCookie(cookieName) != 'shown') { setTimeout(function() { $.magnificPopup.open({ showCloseBtn: false, items: { src: '#subscribe-popup' }, type: 'inline', mainClass: 'my-mfp-zoom-in', callbacks: { close: function() { if ($('#newsletter-off').is(':checked')) { setCookie(cookieName, 'shown', expireDays); } } } }); }, (delaySecond * 3500)); }
        }
    }
    if (typeof $.fn.magnificPopup === 'function') {
        ...
        $('.popup-with-zoom-anim').magnificPopup({ type: 'inline', fixedContentPos: false, fixedBgPos: true, overflowY: 'auto', closeBtnInside: true, preloader: false, midClick: true, removalDelay: 300, blackbg: true, mainClass: 'my-mfp-zoom-in' });
        $('.popup-with-move-anim').magnificPopup({ type: 'inline', fixedContentPos: false, fixedBgPos: true, overflowY: 'auto', closeBtnInside: true, preloader: false, midClick: true, removalDelay: 300, blackbg: true, mainClass: 'my-mfp-slide-bottom' });
        $('.slide-up-animation').magnificPopup({ type: 'inline', fixedContentPos: false, fixedBgPos: true, overflowY: 'auto', closeBtnInside: true, preloader: false, midClick: true, removalDelay: 300, blackbg: true, mainClass: 'my-mfp-slide-up' });
        $('.popup-with-form').magnificPopup({ type: 'inline', preloader: false, fixedContentPos: true, focus: '#name', callbacks: { beforeOpen: function() { if ($(window).width() < 700) { this.st.focus = false; } else { this.st.focus = '#name'; } } } });
    ...
    $(document).on('click', '.submit', function() {
        ...
        if (typeof(grecaptcha) !== 'undefined' && grecaptcha !== null) {
            ...
            } else if (grecaptchav3 != '' && grecaptchav3 != undefined) { grecaptcha.ready(function() { grecaptcha.execute(grecaptchav3, { action: 'submit' }).then(function(token) {}); }); }
        }
        if (!error && actionURL != '' && actionURL != undefined) {
            ...
            $.ajax({
                ...
                success: function(result) {
                    ...
                    if (redirectVal != '' && redirectVal != undefined) { window.location.href = redirectVal; } else {
    ...
    $(document).on('blur', '.required', function() {
        var emailFormat = /^([\w-\.]+@([\w-]+\.)+[\w-]{2,4})?$/,
            ...
            urlformat = /(ftp|http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/,
            fieldVal = $(this).val();
        if (fieldVal == '' || fieldVal == undefined) { $(this).addClass('is-invalid'); } else if ($(this).attr('type') == 'email' && !emailFormat.test(fieldVal)) { $(this).addClass('is-invalid'); } else if ($(this).attr('type') == 'url' && !urlformat.test(fieldVal)) { $(this).addClass('is-invalid'); } else if ($(this).attr('type') == 'tel' && !telFormat.test(fieldVal)) { $(this).addClass('is-invalid'); } else { $(this).removeClass('is-invalid').addClass('is-valid'); }
    });
    $(document).on('click', '.terms-condition', function() { var termsObj = $(this); if (!termsObj.is(':checked')) { termsObj.addClass('is-invalid'); } else { termsObj.removeClass('is-invalid').addClass('is-valid'); } });
    if ($('.rev_slider').length) { $('.rev_slider').each(function() { $(this).one('revolution.slide.onloaded', function() { reInitSkrollr(); }); }); }
    $(window).on('load', function() {
        var tooltipTriggerList = [].slice.call(document.querySelectorAll('[data-bs-toggle="tooltip"]'))
        var tooltipList = tooltipTriggerList.map(function(tooltipTriggerEl) { return new bootstrap.Tooltip(tooltipTriggerEl); });
        if (typeof $.fn.parallax !== 'undefined' && typeof $.fn.parallax !== null) { setParallax(); }
        $("img:not([data-at2x])").each(function() { $(this).attr('data-no-retina', ''); });
        if ($('.page-loader').length) { $('.page-loader').fadeOut(); }
    ...
    $(window).on("load", function() {
        $('.page-loader').show();
        setTimeout(function() { $('.page-loader').hide(); }, 1000);
    });
    $(window).resize(function() {
        ...
        setSpaceInInteractiveBannerStyle();
        if ($('.navbar-collapse-clone').length && typeof $.fn.mCustomScrollbar === 'function') { $('.navbar-collapse-clone').mCustomScrollbar('update'); }
    ...
    function initScrollNavigate() {
        ...
        var navHeight = 0,
            miniHeaderHeight = 0;
        if ($('header nav.navbar').length) { navHeight = $('header nav.navbar').outerHeight(); }
        ...
        if (!$('header').hasClass('no-sticky')) {
            if (scrollPos >= headerHeight) {
                ...
                } else { $('.header-top-bar, .header-top-bar + .navbar').css({ 'top': '' }); }
    ...
    function setupSwiper() {
        ...
        swipers.forEach(function(swiperItem) {
            ...
            if (typeof(sliderOptions) !== 'undefined' && sliderOptions !== null) {
                ...
                if (changeOnClick != '' && changeOnClick != undefined && changeOnClick == '1') { sliderOptions['on'] = { click: function() { if (this.activeIndex > this.clickedIndex) { this.slidePrev(); } else if (this.activeIndex < this.clickedIndex) { this.slideNext(); } } }; }
                ...
                if (numberPagination != '' && numberPagination != undefined && numberPagination == '1' && sliderOptions['pagination'] != '' && sliderOptions['pagination'] != undefined) { sliderOptions['pagination']['renderBullet'] = function(index, className) { return '<span class="' + className + '">' + pad(index + 1) + '</span>'; } }
                var dataThumbs = _this.attr('data-thumbs');
                if (dataThumbs != '' && dataThumbs != undefined && sliderOptions['pagination'] != '' && sliderOptions['pagination'] != undefined) { dataThumbs = $.parseJSON(dataThumbs); if (typeof(dataThumbs) !== 'undefined' && dataThumbs !== null) { sliderOptions['pagination']['renderBullet'] = function(index, className) { return '<span class="' + className + '" style="background-image: url( ' + dataThumbs[index] + ' )"></span>'; } } }
                sliderOptions['on'] = {
                    init: function() {
                        ...
                        slideChange: function() {
                            if (isNumberPagination || isNumberNavigation || isNumberPaginationProgress) {
                                ...
                                var length = this.slides.length,
                                    ...
                                    next = active + 1,
                                    prev = active - 1;
                                if (active == 1) { prev = length; }
                                if (active == length) { next = 1; }
                                if (isNumberPaginationProgress) {
                                    _this.parent().find('.number-prev').each(function() { $(this).text(active < 10 ? '0' + active : active); });
                                    _this.parent().find('.swiper-pagination-progress')[0].style.setProperty('--swiper-progress', ((100 / length) * active).toFixed(2) + '%');
                                } else {
                                    _this.parent().find('.number-next').each(function() { $(this).text(next < 10 ? '0' + next : next); });
                                    _this.parent().find('.number-prev').each(function() { $(this).text(prev < 10 ? '0' + prev : prev); });
                                    if (showProgress)
                                        _this.parent().find('.swiper-pagination-progress')[0].style.setProperty('--swiper-progress', ((100 / length) * active).toFixed(2) + '%');
                                }
                            }
                ...
                if (thumbClick && sliderOptions.hasOwnProperty('thumbs')) { sliderOptions['thumbs']['swiper']['on'] = { click: function(swiper) { if (swiper.activeIndex >= swiper.clickedIndex) { swiper.slidePrev(); } else if (swiper.activeIndex < swiper.clickedIndex) { swiper.slideNext(); } } } }
    ...
    function setOverLayerPosition() {
        if (($('.overlap-section').length > 0 || $('.overlap-section-one-fourth').length > 0 || $('.overlap-section-three-fourth').length > 0)) {
            $('.overlap-section, .overlap-section-one-fourth, .overlap-section-three-fourth').each(function() {
                ...
                if (_this.hasClass('md-overlap-disable')) { overlayBreakpoint = 991; } else if (_this.hasClass('sm-overlap-disable')) { overlayBreakpoint = 767; }
                if (getWindowWidth() > overlayBreakpoint) {
                    setTimeout(function() {
                        _this.imagesLoaded(function() {
                            var closestSectionObj = _this.closest('section');
                            if (_this.closest('footer').length) { closestSectionObj = _this.closest('footer'); }
                            var sectionPaddingTop = parseInt(closestSectionObj.css('padding-top')),
                                areaHeight = _this.find('*').outerHeight(),
                                overlayTop = areaHeight + sectionPaddingTop;
                            if (_this.hasClass('overlap-section-one-fourth')) { overlayTop = (areaHeight / 4) - overlayTop; } else if (_this.hasClass('overlap-section-three-fourth')) { overlayTop = ((areaHeight * 3) / 4) - overlayTop; } else { overlayTop = (areaHeight / 2) - overlayTop; }
    ...
    if ($('.magic-cursor').length > 0) {
        ...
        if (typeof gsap !== "undefined") { gsap.ticker.add(updatePosition); }
    ...
    $(document).on('mouseenter', '.swiper-button-next, .swiper-button-prev, .swiper-pagination, a:not(.force-magic-cursor)', function() { $('.magic-cursor-wrapper').css({ 'opacity': 0 }); }).on('mouseleave', '.swiper-button-next, .swiper-button-prev, .swiper-pagination, a:not(.force-magic-cursor)', function() { $('.magic-cursor-wrapper').css({ 'opacity': 1 }); });
    $(window).scroll(function(event) { $('[data-shadow-animation="true"]').each(function() { addBoxAnimationClass($(this)) }); });
    $('[data-shadow-animation="true"]').removeClass('shadow-in');
    $('[data-shadow-animation="true"]').each(function() { addBoxAnimationClass($(this)) });

    function addBoxAnimationClass(boxObj) { if (boxObj.length) { var w = boxObj.width(); var h = boxObj.height(); var offset = boxObj.offset(); var right = offset.left + parseInt(boxObj.width()); var bottom = offset.top + parseInt(boxObj.height()); var visibleX = Math.max(0, Math.min(w, window.pageXOffset + window.innerWidth - offset.left, right - window.pageXOffset)); var visibleY = Math.max(0, Math.min(h, window.pageYOffset + window.innerHeight - offset.top, bottom - window.pageYOffset)); var visible = visibleX * visibleY / (w * h); if (visible >= 0.5) { if (typeof boxObj.attr('data-animation-delay') !== 'undefined' && boxObj.attr('data-animation-delay') > 10) { setTimeout(function() { boxObj.addClass('shadow-in'); }, boxObj.attr('data-animation-delay')); } else { boxObj.addClass('shadow-in'); } } } }
    var skroller;

    function initSkrollr() { if (typeof skrollr !== 'undefined' && typeof skrollr !== null) { skroller = skrollr.init({ 'forceHeight': false, 'smoothScrollingDuration': 1000, 'mobileCheck': function() { return false; } }); } }
    ...
    if (typeof particlesJS === 'function' && particleItems.length) {
        $.each(particleItems, function(index, particleItem) {
            ...
            if (particleItemOptions != 'undefined' && typeof particleItemOptions !== typeof undefined) { particleItemOptions = JSON.parse(particleItemOptions); } else { particleItemOptions = particleDefaultOptions; }
                ...
                if (stackItems.length) {
                    ...
                    for (let i = 0; i < stackItems.length - 1; i++) {
                        ...
                        if ((windowScrollTop - stackTopPosition) > stackBoxHeight) {
                            ...
                            if (yMove > _this.outerHeight()) { yMove = _this.outerHeight(); }
    ...
    $(window).scroll(function() { stackAnimation(); });
    ...
    $(window).on('scroll resize load', function(e) { getElementsInArea(e, { elements: document.querySelectorAll('.section-dark'), markedClass: 'section-dark-highlight', zone: [10, 10] }); if ($('.section-dark-highlight').length > 0) { $('.sticky-wrap').addClass('sticky-highlight'); } else { $('.sticky-wrap').removeClass('sticky-highlight'); } });
    $(document).on('appear', 'footer', function(e) { $('.sticky-wrap').addClass('sticky-hidden'); }).on('disappear', 'footer', function(e) { $('.sticky-wrap').removeClass('sticky-hidden'); });
    ...
    function createEl(i) {
        ...
        tl.add({ begin: function() { anime({ targets: el, rotate: [rotate + 'deg', rotate + 10 + 'deg'], translateY: [translateY + '%', translateY + 10 + '%'], scale: [1, 1.25], easing: 'easeInOutSine', direction: 'alternate', duration: duration * .1 }); } });
        if (wrapperEl)
    ...
    function onScroll() {
        if (window.sections.length > 0) {
            const section = window.sections.map(section => { const el = section; const rect = el.getBoundingClientRect(); return { el, rect }; }).find(section => section.rect.bottom >= (window.innerHeight * 0.5));
    ...
    if (typeof $.cookie === 'function') {
        if ($('body').find('#cookies-model').length > 0) {
            setTimeout(function() {
                ...
                if (cookieConsentclosed == 'closed') { cookieModel.remove(); } else { cookieModel.show(); }
    ...
    $(document).on('click', '.scroll-top', function() { $('html, body').animate({ scrollTop: 0 }, 800); return false; });

    function scrollIndicator() {
        var scrollTop = document.documentElement.scrollTop;
        if (scrollTop > 200) { $('.scroll-progress').addClass('visible'); } else { $('.scroll-progress').removeClass('visible'); }
        ...
        var scrollPos = $(window).scrollTop();
        if (scrollPos > 150) { $('.theme-demos').fadeIn(600); }
    }
    $(window).scroll(function() { scrollIndicator(); });
    ...
    $(document).on('click', '.close-popup', function() {
        ...
        $('body').removeClass('overflow-hidden');
        clearTimeout(timer);
        timer = setTimeout(function() { $('.theme-demos-main').animate({ scrollTop: '0' }, 'slow'); }, 800);
    });
    ...
    $(".all-demo a").click(function(event) { event.preventDefault(); });
})(jQuery);
...
function initMap() {
    $('.map').each(function(e) {
        ...
        if (typeof(mapOptions) !== 'undefined' && mapOptions !== null) { mapOptions = $.parseJSON(mapOptions); }
